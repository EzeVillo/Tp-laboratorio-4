...
class Pago extends Model
{
    ...
    public function NuevoPago($id_solicitud_de_reparacion, $monto, $id_metodo_de_pago)
    {
        ...
        if ($this->db->fetch()["c"] != 1) throw new Exception("No existe la solicitud de reparacion con ese id");
        ...
        if ($this->db->fetch()["c"] != 1) throw new Exception("No existe ese metodo de pago");

        if (!ctype_digit($monto)) throw new Exception("El monto debe ser un numero");
